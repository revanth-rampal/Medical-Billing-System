...
class BillingSystem:
    def __init__(self, root, cursor_patient_details):
        ...
        self.columns = ("id", "medication name", "price", "quantity", "PRICE", "GST", "Total")  # Added Total column
    ...
    def add_medication_to_bill(self):
        ...
        if medication_details:
            ...
            # Update the treeview
            self.treeview.insert("", "end", values=(medication_id, medication_name, price, quantity, single_price, gst, total_price))
    ...
    def remove_medication_from_bill(self):
        ...
        if selected_item:
            ...
            total_price = item_values[6]  # Total price is at index 6
    ...
    def print_receipt(self):
        try:
            ...
            class PDF(FPDF):
                def header(self):
                    # Add header image
                    header_image = Image.open("bill.png")  # Path to your header image
                    width, height = header_image.size
                    aspect_ratio = height / width
                    new_width = 200  # Adjust width as needed, keeping aspect ratio
                    new_height = int(new_width * aspect_ratio)
                    header_image = header_image.resize((new_width, new_height))
                    self.image("bill.png", 10, 8, 180)  # Adjust x, y, and width as needed
            ...
            col_widths = [10, 30, 30, 30, 30, 20, 30]  # Modified for the Total column
            col_names = ["id", "medication name", "price", "quantity", "PRICE", "GST", "Total"]  # Added Total column
            ...
            # Add total price
            pdf.cell(sum(col_widths[:-1]), 10, txt="Total Price", border=1, align='C')
            pdf.cell(col_widths[-1], 10, txt="{:.2f}".format(self.total_price), border=1, align='C')
            pdf.ln()
            ...
            # Update the inventory database and commit
            for item in self.bill:
