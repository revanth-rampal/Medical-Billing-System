...
class BillingSystem:
    def __init__(self, root):
        ...
        self.create_inventory_history_table()
    ...
    def create_inventory_history_table(self):
        try:
        ...
        try:
            with sqlite3.connect('inventory.db') as conn:
                ...
                for item in self.bill:
                    ...
                    # Insert purchase details into inventory_history database
                    current_date = datetime.now().strftime('%d/%m/%Y')  # Get current date in DD/MM/YYYY format
                    cursor.execute("INSERT INTO InventoryHistory (MedicationID, MedicationName, Quantity, ExpirationDate, LotNumber, Location, ADDED, Price, CurrentDate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                              (medication_id, _, quantity, _, _, _, 'PURCHASED', _, current_date))
        except sqlite3.Error as e:
            messagebox.showerror("Database Error", str(e))
        else:
